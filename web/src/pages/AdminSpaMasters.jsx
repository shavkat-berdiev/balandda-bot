...
export default function AdminSpaMasters() {
  ...
  async function load() {
    setLoading(true);
    try { setItems(await api.getSpaMasters()); } catch (err) { setError(err.message); }
    setLoading(false);
  }
  ...
  async function toggleActive(it) {
    try { await api.updateSpaMaster(it.id, { is_active: !it.is_active }); load(); } catch (err) { setError(err.message); }
  }
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">SPA мастера</h1>
          <p className="text-gray-500 text-sm mt-1">Мастера назначаются на услуги в разделе «Услуги»</p>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">{editId ? 'Редактировать' : 'Новый мастер'}</h2>
