...
export default function AdminServiceCategories() {
  ...
  async function load() {
    setLoading(true);
    try { setItems(await api.getServiceCategories()); } catch (err) { setError(err.message); }
    setLoading(false);
  }
  ...
  async function handleSave() {
    try {
      ...
      if (editId) await api.updateServiceCategory(editId, payload);
      else await api.createServiceCategory(payload);
      setShowForm(false); load();
    } catch (err) { setError(err.message); }
  }

  async function toggleActive(it) {
    try { await api.updateServiceCategory(it.id, { is_active: !it.is_active }); load(); } catch (err) { setError(err.message); }
  }
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">SPA категории</h1>
          <p className="text-gray-500 text-sm mt-1">Группы услуг (сортируемые)</p>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">{editId ? 'Редактировать' : 'Новая категория'}</h2>
